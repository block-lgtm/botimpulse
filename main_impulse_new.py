...
EXCEL_STRAT_START_COL = 14
CHAT_ID   = os.getenv("CHAT_ID")
BOT_TOKEN = os.getenv("BOT_TOKEN")

client = Client()
BLACKLIST = {"BTCUSDT","ETHUSDT","BNBUSDT","SOLUSDT","XRPUSDT","ADAUSDT","DOGEUSDT","LINKUSDT"}
...
SHEET_MAP = {
    "CONFIMP1":"confimp1","CONFIMP2":"confimp2","CONFIMP3":"confimp3","CONFIMP4":"confimp4",
    "CONFIMP5":"confimp5","CONFIMP6":"confimp6","CONFIMP7":"confimp7","CONFIMP8":"confimp8",
    "CONFIMP9":"confimp9","CONFIMP10":"confimp10","CONFIMP11":"confimp11","CONFIMP12":"confimp12",
    "CONFIMP13":"confimp13","CONFIMP14":"confimp14","CONFIMP15":"confimp15","CONFIMP16":"confimp16",
    "CONFIMP17":"confimp17","CONFIMP18":"confimp18",
}
...
def load_trade_id():
    if not os.path.exists(TRADE_STATE_FILE): return 0
    with open(TRADE_STATE_FILE,"r") as f: return json.load(f).get("last_trade_id",0)

def save_trade_id(tid):
    with open(TRADE_STATE_FILE,"w") as f: json.dump({"last_trade_id":tid},f)

def save_active_trades():
    with TRADES_LOCK:
        with open(ACTIVE_TRADES_FILE,"w") as f: json.dump(ACTIVE_TRADES,f)
    ...
    with EXCEL_LOCK:
        if not os.path.exists(EXCEL_FILE):
            ...
            for sn in SHEET_MAP.values():
                if sn not in wb.sheetnames: wb.create_sheet(sn)
            if "Sheet" in wb.sheetnames: wb.remove(wb["Sheet"])
            wb.active = wb[sheet_name]; wb.save(EXCEL_FILE)
    ...
    with EXCEL_LOCK:
        ...
        for row in range(2, ws.max_row+1):
            if str(ws[f"F{row}"].value)==trade_id:
                col_s = {"3:1":"N","6:1":"O","6:2":"P","10:3":"Q","12:4":"R"}[strategy_name]
                col_d = {"3:1":"S","6:1":"T","6:2":"U","10:3":"V","12:4":"W"}[strategy_name]
...
def get_liquid_futures_symbols():
    ...
    for t in tickers:
        symbol = t["symbol"]
        if not symbol.endswith("USDT") or symbol in BLACKLIST: continue
        if float(t["quoteVolume"]) < MIN_24H_VOLUME: continue
        symbols.append(symbol)
    return symbols

def get_symbols_with_open_trades():
    """Возвращает символы у которых есть хотя бы одна OPEN стратегия."""
    with TRADES_LOCK:
        return {
...
def get_btc_returns():
    try:
        klines_btc = client.futures_klines(symbol="BTCUSDT",interval=Client.KLINE_INTERVAL_1HOUR,limit=BTC_LOOKBACK)
        df_btc = pd.DataFrame(klines_btc,columns=["open_time","open","high","low","close","volume","close_time","quote_volume","trades","taker_buy_base","taker_buy_quote","ignore"])
        df_btc["close"] = df_btc["close"].astype(float)
        return df_btc["close"].pct_change()
    except Exception as e:
        print(f"Ошибка загрузки BTC свечей: {e}"); return None
...
def check_swing(df, side, n):
    ...
    for i in range(1,n+1):
        ...
        candle = df.iloc[idx]
        if side=="BUY" and candle["low"]<current["low"]: return False
        if side=="SELL" and candle["high"]>current["high"]: return False
    return True

def get_swing_num(df, side, n=5):
    current = df.iloc[-2]; result = 0
    for i in range(1,n+1):
        idx = -2-i
        if abs(idx)>len(df): break
        candle = df.iloc[idx]
        if side=="BUY" and candle["low"]<current["low"]: result=i
        if side=="SELL" and candle["high"]>current["high"]: result=i
    return result

def check_volume_signal(symbol):
    klines = client.futures_klines(symbol=symbol,interval=Client.KLINE_INTERVAL_1HOUR,limit=LOOKBACK_CANDLES)
    ...
    bull = last["close"]>last["open"]; bear = last["close"]<last["open"]
    ...
    bull_trend = last["ema20"]>last["ema200"]; bear_trend = last["ema20"]<last["ema200"]
    atr = last["atr"]
    ...
    signals = []
    if spike_trend and bull and strong_body_trend and below_ema20 and below_vwap and bull_trend and emas_far_enough and buy_low_condition and ema20_far_vwap and swing_buy_trend_ok: signals.append("BUY_TREND")
    if spike_trend and bear and strong_body_trend and above_ema20 and above_vwap and bear_trend and emas_far_enough and sell_high_condition and ema20_far_vwap and swing_sell_trend_ok: signals.append("SELL_TREND")
    if spike_counter and bull and strong_body_counter and below_ema20 and below_vwap and bear_trend and emas_far_enough and ema20_clear_zone and swing_buy_counter_ok: signals.append("BUY_COUNTER")
    if spike_counter and bear and strong_body_counter and above_ema20 and above_vwap and bull_trend and emas_far_enough and ema20_clear_zone and swing_sell_counter_ok: signals.append("SELL_COUNTER")
    ...
    return {"symbol":symbol,"signals":signals,"close":last["close"],"low":last["low"],"high":last["high"],"ema20":last["ema20"],"ema200":last["ema200"],"vwap":last["vwap"],"natr":round(last["natr"],3),"volText":f"x{last['quote_volume']/avg_vol:.2f}","prevVolCount":int((df.iloc[-5:-2]["quote_volume"]>last["quote_volume"]).sum()),"volume_24h":volume_24h,"swing_num":swing_num}
...
def main():
    ...
    def update_symbols_periodically():
        ...
        while True:
            ...
            try:
                ...
                open_symbols = get_symbols_with_open_trades()
                symbols = list(set(liquid) | open_symbols)
                print(f"♻️ Обновление токенов: {len(liquid)} ликвидных + {len(open_symbols)} с открытыми позициями")
            except Exception as e: print(f"Ошибка обновления токенов: {e}")
    ...
    def process_signal(msg):
        try:
            if msg.get("e")=="error":
                ...
                # Отправляем в TG только если просит перезапустить соединение
                if "reset" in str(err_msg).lower() or "closed" in str(err_msg).lower():
                    send_telegram(f"🔴 {BOT_NAME} WebSocket ошибка: {err_msg}")
                return
            if 'data' not in msg or 'k' not in msg['data']: return
            candle = msg['data']['k']
            symbol = candle['s']
            if not candle['x']: return

            price_high = float(candle["h"]); price_low = float(candle["l"])
            closed_trades = []
            with TRADES_LOCK:
                for trade_id,trade in list(ACTIVE_TRADES.items()):
                    if trade["symbol"]!=symbol: continue
                    for strat_name,strat in trade["strategies"].items():
                        if strat["status"]!="OPEN": continue
                        if trade["side"]=="BUY":
                        ...
                        Thread(target=update_trade_status_in_excel,args=(trade_id,strat_name,result,close_price),daemon=True).start()
                    if all(s["status"]!="OPEN" for s in trade["strategies"].values()): closed_trades.append(trade_id)
                for tid in closed_trades: del ACTIVE_TRADES[tid]
            if closed_trades: save_active_trades()

            # Новые сигналы только для символов из основного списка
            if symbol not in symbols: return
            now = time.time()
            if now-last_signal_time.get(symbol,0)<cooldown_seconds: return
            ...
            try:
                ...
                if btc_returns is not None:
                    klines_sym=client.futures_klines(symbol=symbol,interval=Client.KLINE_INTERVAL_1HOUR,limit=BTC_LOOKBACK)
                    df_sym=pd.DataFrame(klines_sym,columns=["open_time","open","high","low","close","volume","close_time","quote_volume","trades","taker_buy_base","taker_buy_quote","ignore"])
                ...
                print(f"Ошибка корреляции {symbol}: {e}"); corr_text="N/A"
            trade_id=get_next_trade_id()
            strategies={}
            for name,strat_cfg in STRATEGIES.items():
                if side=="BUY": tp=entry_price*(1+strat_cfg["tp"]); sl=entry_price*(1-abs(strat_cfg["sl"]))
                else: tp=entry_price*(1-strat_cfg["tp"]); sl=entry_price*(1+abs(strat_cfg["sl"]))
                strategies[name]={"tp":tp,"sl":sl,"status":"OPEN"}
            with TRADES_LOCK:
                ACTIVE_TRADES[trade_id]={"symbol":symbol,"side":side,"entry_price":entry_price,"strategies":strategies,"open_time":datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")}
            save_active_trades()
            write_trade_to_excel(trade_id,{"symbol":symbol,"signals":res["signals"],"strategies":strategies,"entry_price":entry_price,"natr":res["natr"],"swing_num":res["swing_num"]},vol_text=res["volText"],vol24=res["volume_24h"]/1_000_000,corr_text=corr_text)
            vol24=res["volume_24h"]/1_000_000
            msg_text=(f"🤖 {BOT_NAME}\n🔥 {res['symbol']}\nТип: {', '.join(res['signals'])}\nClose: {res['close']:.6f}\nEMA20: {res['ema20']:.6f}\nEMA200: {res['ema200']:.6f}\nVWAP: {res['vwap']:.6f}\nVOL {res['volText']}\nPrev volume higher: {res['prevVolCount']}/3\nVOL 24h: {vol24:.1f}M USDT\nCorr BTC: {corr_text}\nNATR: {res['natr']}%\nСвинг: {res['swing_num']}\n")
            print(msg_text)
        except Exception as e: print(f"Ошибка process_signal: {e}")

    def handle_kline(msg): task_queue.put(msg)
    def worker():
        while True:
            msg=task_queue.get(); process_signal(msg); task_queue.task_done()
    ...
    while True:
        try:
            # Добавляем символы с открытыми позициями перед запуском
            open_symbols = get_symbols_with_open_trades()
            all_symbols = list(set(symbols) | open_symbols)
            ...
            # В TG только если не "Socket Manager failed" (это нормальный перезапуск)
